 ...
 *
 * Created on November 28, 2022, 8:46 PM
 */
#include "IOs.h"
#include "TimeDelay.h"
#include "UART2.h"
...
void IOCheck(void)
{
    ...
    {
        float ADCValue = do_ADC(state) / 1024.0;//get adc value and divide by 1023.0 to get decimal value
        
        if(state == 1)  //if in voltmeter mode
        {
            valueToDisplay = ADCValue * 3.25;// Voltage = (ADC / 1024) * VDD
            ...
            Disp2String("Voltage From Voltmeter = ");//print text to screen
        ...
        else//if in ohmmeter mode
        {
            ...
            NewClk(8);//change clk for smooth printing
            Disp2String("Resistance From Ohmmeter = ");//print text to screen
